...
let dataset = await import(
  "../Data/" +
    global.wdioEnvParameters.config.appName +
    "/Physicalinventory.json",
  { assert: { type: "json" } }
);
let locators = await import(
  "../pageobjects/elementIdentifiers/" +
    global.wdioEnvParameters.config.appName +
    "/physicalinventoryxpath.json",
  { assert: { type: "json" } }
);
...
describe("Create Create Maintenance Notification and Order", () => {
  ...
  it("Should Create Maintenance Notification and Order", async () => {
    ...
    try {
      //jeeva changes end
      await Base.waitForDisplayedAndSetValue(
        $(locators.Usernameoktaxpath),
        dataset.logincred.username
      );
      await Base.waitForDisplayedAndSetValue(
        $(locators.Passwordoktaxpath),
        dataset.logincred.password
      );
      await Base.waitForDisplayedAndClick(
        $(locators.Submitbuttonxpath),
        dataset.logincred.submitbtn,
        5000
      );
      await takeScreenshot("login_success");

      await Base.waitForDisplayedAndSetValue(
        $(locators.Codepathxpath),
        dataset.logincred.code
      ); // OTP_____________________
      await Base.waitForDisplayedAndClick($(locators.verifybuttonxpath), 5000);
      await browser.pause(10000);

      const dashboardElement = await $(
        "//section[@id='main-content']/child::section/section/descendant::section[35]"
      );
      const CreateMaintenanceNotification = $(
        '//*[@id="dashboardGroups"]/div/child::div/descendant::ul/descendant::div[289]/child::div/div'
      );
      const CreateMaintenanceOrder = $(
        '//*[@id="dashboardGroups"]/div/child::div/descendant::ul/descendant::div[299]/child::div/div'
      );
      ...
      const iframePick = await browser.$(
        '//iframe[@id="application-Shell-startGUI"]'
      );
      ...
      await Base.waitForDisplayedAndSetValue(
        ...
        dataset.MTSNOftification.NotificationType
      );
      ...
      const enterNotificationDiscription = await $(
        '//input[@id="M0:46:1:1::0:10"]'
      );
      await Base.waitForDisplayedAndSetValue(
        enterNotificationDiscription,
        dataset.MTSNOftification.NotificationDiscription
      );
      await takeScreenshot("Enter_notification_description");
      await browser.pause(5000);

      // click on Priority Dropdown
      const clickPriorityDropdown = await $(
        '//input[@id="M0:46:2:3B256:1:4::1:55"]'
      );
      ...
      await Base.waitForDisplayedAndSetValue(
        enterActType,
        dataset.MTSNOftification.ActType
      );
      await browser.pause(5000);

      // enter Functional Location
      const enterFunctionalLocation = await $(
        '//input[@id="M0:46:2:3B256:1:1:1::0:16"]'
      );
      await Base.waitForDisplayedAndSetValue(
        enterFunctionalLocation,
        dataset.MTSNOftification.FunctionalLocation
      );
      await browser.pause(5000);

      // enter Equipment
      const enterEquipment = await $(
        '//input[@id="M0:46:2:3B256:1:1:1::1:16"]'
      );
      await Base.waitForDisplayedAndSetValue(
        enterEquipment,
        dataset.MTSNOftification.Equipment
      );
      ...
      const CreateNotification = await $(
        "//div[@id='msgarea']/child::div[1]/div/descendant::span[3]"
      ).getText();
      ...
      const clickOnNotificationHeader = await $(
        '//div[@title="Notification (F8)"]'
      );
      ...
      const clickOnManinNotification = await $(
        '//span[text()="Maintenance notification"]'
      );
